package com.surge.common.mybatis.core.mapper;
...
public interface BaseMapperPlus<M, T, V> extends BaseMapper<T> {
    ...
    default <P extends PageInfo<V>> P selectVoPage(PageInfo<T> page, Wrapper<T> wrapper) {
     ...
     * 分页查询VO
     */
    default <C, P extends PageInfo<C>> P selectVoPage(PageInfo<T> page, Wrapper<T> wrapper, Class<C> voClass) {
        List<T> list = this.selectList(page, wrapper);
        PageInfo<C> voPage = new PageInfo<>(page.getCurrent(), page.getSize(), page.getTotal());
        if (CollUtil.isEmpty(list)) {
            return (P) voPage;
        }
        voPage.setRecords(BeanUtils.copyList(list, voClass));
        return (P) voPage;
    }
